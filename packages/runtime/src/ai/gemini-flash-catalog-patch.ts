 ...
 * baked Google catalog, so the `google` provider offers them and a turn can
 * run on them.
 *
 ...
 *
 * Idempotent: a no-op once pi-ai serves them natively. The host has a twin
 * (packages/host/src/providers/gemini-flash-catalog-patch.ts) because host
 * and runtime are separate processes — DELETE BOTH when the pi bump lands.
 */
