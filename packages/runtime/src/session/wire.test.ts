...
test("normalizeUsage: context_tokens = totalTokens - output; cached = cacheRead", () => {
  ...
  expect(
    normalizeUsage({ input: 100, output: 20, cacheRead: 300, cacheWrite: 50, totalTokens: 470 }),
...
test("toWire maps turn_end (with usage) to a usage frame", () => {
  expect(
    toWire({
      type: "turn_end",
      turnIndex: 0,
      message: { usage: { input: 100, output: 20, cacheRead: 300, cacheWrite: 50, totalTokens: 470 } },
      toolResults: [],
    }),
  ).toEqual({ type: "usage", data: { context_tokens: 450, output_tokens: 20, cached_tokens: 300 } });
});
