...
4. Call \`custom_integration_add\` with what you learned. Pick a friendly name the user will recognize. The result tells you how many actions compiled: check that number against the operations the documentation describes. If it is lower than what you authored or expected, the spec is wrong — fix it and call \`custom_integration_add\` again with \`replace: true\` (same name; the user's saved key survives) until the count matches, BEFORE telling the user it is ready. Never present a partial integration as done, and never create a second integration for the same service to paper over a bad first spec.
