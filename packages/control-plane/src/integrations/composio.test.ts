...
function harness(handler: (url: URL, method: string) => Reply) {
  ...
  const provider = new ComposioProvider({ baseURL: "https://cmp.test", fetch: fetchImpl });
...
test("verifyCredential maps session/info and sends the user key", async () => {
  const { provider, calls } = harness((url) => {
    if (url.pathname === "/api/v3/auth/session/info") {
      return { body: { org_member: { user_id: "consumer-1", email: "a@b.com" } } };
    }
    return { status: 404 };
  });
  expect(await provider.verifyCredential(cred)).toEqual({ accountId: "consumer-1", email: "a@b.com" });
...
test("listToolkits maps the catalog", async () => {
  const { provider, calls } = harness((url) =>
    url.pathname === "/api/v3/toolkits"
      ? { body: { items: [{ slug: "gmail", name: "Gmail", meta: { description: "Email", logo: "l.png" } }] } }
      : { status: 404 },
  );
  const toolkits = await provider.listToolkits(cred);
  expect(toolkits).toEqual([{ slug: "gmail", name: "Gmail", description: "Email", logoUrl: "l.png", categories: [] }]);
...
test("execute posts user_id + arguments to the action path and maps the result", async () => {
  const { provider, calls } = harness((url, method) =>
    method === "POST" && url.pathname === "/api/v3/tools/execute/GMAIL_SEND_EMAIL"
      ? { body: { successful: true, data: { id: "msg1" } } }
      : { status: 404 },
  );
  const result = await provider.execute(cred, "GMAIL_SEND_EMAIL", { to: "a@b.com", subject: "Hi" });
  expect(result).toEqual({ successful: true, data: { id: "msg1" }, error: undefined });
  expect(calls[0]!.body).toEqual({ user_id: "consumer-1", arguments: { to: "a@b.com", subject: "Hi" } });
});
...
test("disconnect deletes every connected account for the toolkit", async () => {
  ...
  expect(deletes).toEqual(["/api/v3/connected_accounts/ca1", "/api/v3/connected_accounts/ca2"]);
});

test("search maps discovered actions to slugs + param schemas", async () => {
  const { provider, calls } = harness((url) =>
    url.pathname === "/api/v3/tools"
      ? { body: { items: [{ slug: "GMAIL_SEND_EMAIL", toolkit: { slug: "gmail" }, description: "Send", input_parameters: { type: "object" } }] } }
      : { status: 404 },
  );
  const matches = await provider.search(cred, "email");
  expect(matches).toEqual([
    { action: "GMAIL_SEND_EMAIL", toolkit: "gmail", description: "Send", inputParams: { type: "object" } },
  ]);
...
test("a credential for another provider, or missing apiKey, is rejected", async () => {
  ...
  await expect(provider.execute({ provider: "other", data: { apiKey: "x", userId: "u" } }, "A", {})).rejects.toThrow(
    /not 'composio'/,
  );
  await expect(provider.execute({ provider: "composio", data: { userId: "u" } }, "A", {})).rejects.toThrow(
    /missing 'apiKey'/,
  );
});
