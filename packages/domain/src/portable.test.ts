...
import { packAgent, portableInventory, unpackAgent, type PortableContent } from "./portable";
...
function content(): PortableContent {
  return {
    ...
    skills: [
      { slug: "research", body: skillBody("research", "Deep-dive on a company") },
      { slug: "weekly", body: skillBody("weekly", "Write the weekly report") },
    ],
    routines: [createRoutine({ name: "Daily", prompt: "check", schedule: "0 9 * * *" }, "r1", NOW)],
    learnings: [{ id: "l1", text: "The user prefers concise updates.", created_at: NOW }],
  };
}

const meta = { agentName: "Sales", description: "A sales helper", houstonVersion: "0.5.0" };

test("pack → unpack round-trips every part", () => {
  const bytes = packAgent(content(), meta, NOW);
  const pkg = unpackAgent(bytes);

  expect(pkg.manifest).toMatchObject({ agentName: "Sales", houstonVersion: "0.5.0", formatVersion: 1 });
...
test("the inventory preview reflects what's inside (skill descriptions from frontmatter)", () => {
  ...
  expect(inv.routines).toEqual([{ id: "r1", name: "Daily", schedule: "0 9 * * *" }]);
  expect(inv.learnings).toEqual([{ id: "l1", text: "The user prefers concise updates." }]);
});
...
test("a CLAUDE.md-only agent (no skills/routines) round-trips", () => {
  const c: PortableContent = { claudeMd: "just instructions", skills: [], routines: [], learnings: [] };
...
test("unpacking junk bytes throws a clear error", () => {
  expect(() => unpackAgent(new Uint8Array([1, 2, 3, 4]))).toThrow("not a valid .houstonagent");
});
...
test("a future format version is rejected with an upgrade hint", () => {
  ...
  const bytes = zipSync({
    "manifest.json": strToU8(JSON.stringify({ agentName: "X", houstonVersion: "9", createdAt: NOW, formatVersion: 99 })),
  });
...
test("malformed routine/learning entries are dropped on unpack", () => {
  const { zipSync, strToU8 } = require("fflate");
  const bytes = zipSync({
    "manifest.json": strToU8(JSON.stringify({ agentName: "X", houstonVersion: "1", createdAt: NOW, formatVersion: 1 })),
    "routines.json": strToU8(JSON.stringify([{ id: "ok", name: "R", schedule: "* * * * *" }, { junk: true }])),
