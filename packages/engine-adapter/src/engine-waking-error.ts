...
// BOTH the desktop app and the web shell (each depends on the workspace
// package, resolved through pnpm's symlink — there is no alias), and only the
// web build can resolve `@houston/app/*`. An adapter import of app code broke
// every desktop release build while the web build passed.
// app/src/lib/engine-waking-error.ts re-exports this module for the app's own
// call sites.
//
...
// transport (`packages/engine-adapter/src/cp/transient-retry.ts`) parses
