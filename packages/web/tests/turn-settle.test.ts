...
import {
  ...
  TURN_DIED_MESSAGE,
  type TurnState,
} from "../src/engine-adapter/turn-settle";
...
function run(
  ...
  const s = newTurnState("Houston/Bo", "activity-settle");
  ...
  const off = bus.on((e) => {
    ...
    if (ev.data?.session_key !== "activity-settle") return;
    if (ev.type === "FeedItem" && ev.data.item) items.push(ev.data.item);
    if (ev.type === "SessionStatus" && ev.data.status)
      statuses.push([ev.data.status, ev.data.error]);
  });
