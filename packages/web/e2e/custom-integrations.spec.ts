...
test("the add fork leads with the guided chat and keeps the manual form beside it", async ({
  page,
  request,
}) => {
  await armCapabilities(request, { integrations: ["composio", "custom"] });
  ...
  // Both paths are visible without hovering, and the guided chat comes first.
  const dialog = page.getByRole("dialog");
  const chat = dialog.getByRole("button", { name: /Set up with your agent/ });
  const manual = dialog.getByRole("button", { name: /Add manually/ });
  await expect(chat).toBeVisible();
  await expect(manual).toBeVisible();
  const chatBox = await chat.boundingBox();
  const manualBox = await manual.boundingBox();
  expect(chatBox?.y ?? 0).toBeLessThan(manualBox?.y ?? 0);

  // The manual path opens the typed form in place; Back returns to the fork.
  await manual.click();
  await expect(dialog.getByLabel("API documentation URL")).toBeVisible();
