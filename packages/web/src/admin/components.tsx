...
export function StatCards({ overview }: { overview: Overview }) {
  ...
  const items = [
    ...
    { label: "Running pods", value: `${t.pods.running} / ${t.pods.total}`, hint: `${t.pods.pending} pending` },
    { label: "Burn (est.)", value: `${usd(t.cost.perHourUsd)}/hr`, hint: `≈ ${usd(t.cost.perMonthUsd)}/mo` },
  ];
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 12 }}>
      {items.map((it) => (
        <div key={it.label} style={card}>
          <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 0.4, color: C.faint }}>
            {it.label}
          </div>
          <div style={{ fontSize: 26, fontWeight: 700, marginTop: 6 }}>{it.value}</div>
          {it.hint && <div style={{ fontSize: 12, color: C.dim, marginTop: 2 }}>{it.hint}</div>}
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ ...card, marginTop: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12, marginTop: 14 }}>
        <Metric label="Estimated monthly run-rate" value={`${usd(e.total.perMonthUsd)}/mo`} sub={`${usd(e.total.perHourUsd)}/hr right now`} />
        <Metric label="Cluster management fee" value={`${usd(e.clusterFeeMonthUsd)}/mo`} sub="flat; offset by the GKE free tier" />
        {billing.actuals ? (
          <Metric
          ...
          <Metric label={`Actual billed (last ${days}d)`} value="—" sub={actualsHint(billing)} accent={billing.actualsStatus === "error" ? C.red : C.faint} />
        )}
      </div>

      <div style={{ fontSize: 12, color: C.dim, marginTop: 12, lineHeight: 1.5 }}>{billing.note}</div>
      {billing.actualsStatus === "error" && billing.actualsError && (
        <div style={{ fontSize: 12, color: C.red, marginTop: 6 }}>BigQuery error: {billing.actualsError}</div>
      )}
    </div>
  );
}
...
function actualsHint(billing: BillingReport): string {
  if (billing.actualsStatus === "error") return "BigQuery query failed (see below)";
...
function Metric({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: string }) {
  return (
    <div style={{ background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 10, padding: 14 }}>
      <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 0.4, color: C.faint }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 700, marginTop: 6, color: accent ?? C.text }}>{value}</div>
...
export function UsersTable({ overview, billing }: { overview: Overview; billing: BillingReport | null }) {
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {overview.users.length === 0 && (
            <tr><td style={{ ...td, color: C.dim }} colSpan={5}>No users yet.</td></tr>
          )}
          {overview.users.map((u) => (
            <UserRow key={u.workspaceId} u={u} actual={actualByNs.get(u.namespace)} />
          ))}
        </tbody>
      </table>
    </div>
...
function UserRow({ u, actual }: { u: UserView; actual?: number }) {
  ...
  return (
    <>
      ...
      >
        <td style={td}>
          <span style={{ color: C.faint, marginRight: 6 }} aria-hidden>{open ? "▾" : "▸"}</span>
          <span style={{ fontFamily: "ui-monospace, monospace", fontSize: 12 }}>{u.namespace}</span>
          <div style={{ fontSize: 11, color: C.faint, marginLeft: 18 }}>{u.userId}</div>
        </td>
        ...
        <td style={{ ...td, textAlign: "right", fontWeight: 600 }}>
          ...
          {actual != null && <div style={{ fontSize: 11, color: C.green }}>{usd(actual)} billed</div>}
        </td>
      </tr>
      {open &&
        u.agents.map((a) => (
      ...
      {open && u.agents.length === 0 && (
        <tr style={{ background: C.panel2 }}>
          <td style={{ ...td, paddingLeft: 28, color: C.dim }} colSpan={5}>No agents.</td>
        </tr>
      )}
    </>
  );
  ...
  return (
    <div style={{ ...card, marginTop: 16, borderColor: `${C.amber}66` }}>
      ...
      <div style={{ fontSize: 12, color: C.dim, marginTop: 4 }}>
        Managed pods/volumes that match no current agent — likely leaked by a failed delete. Worth cleaning up.
      </div>
      <div style={{ marginTop: 10, fontFamily: "ui-monospace, monospace", fontSize: 12, color: C.dim }}>
        {o.pods.map((p) => (
