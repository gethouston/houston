...
test("turnErrorMessage unwraps the engine's plain message from a rejected send", () => {
  ...
  const err = new EngineError(409, JSON.stringify({ error: "No provider connected. Log in with Claude or Codex first." }));
  expect(turnErrorMessage(err)).toBe("No provider connected. Log in with Claude or Codex first.");
});
