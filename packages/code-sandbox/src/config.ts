...
export const config = {
  ...
  /**
   * The ONLY way an empty {@link token} means "open". Behind the gateway relay
   * the sandbox answers whatever reaches it, so an unset token must fail closed
   * (401) instead of quietly serving every caller — a missing secret is a
   * deploy mistake, not a decision. Local dev sets this to 1 on purpose.
   */
  allowUnauthenticated: env.SANDBOX_ALLOW_UNAUTHENTICATED === "1",
