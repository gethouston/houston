...
- Sidebar TEAMS + the `team` screen (teams-model, `openTeamView` store contract, `visibleTeamSections` gating, the one-sweep team-scoped board, Team Settings drill-in) → `knowledge-base/teams-ui.md` (client-side grouping; NOT the multiplayer orgs in `teams.md`)
...
- The ONE per-agent settings surface (section model + caps gating, the shared rail the Permissions drill-in AND the Context/Admin tabs render, the People "everyone vs specific people" choice, read-only, share analytics) → `knowledge-base/agent-settings.md`
