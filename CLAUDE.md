...
- **`@houston/host-cloud`** *(no longer in this repo)* — the **CLOSED** cloud-adapter package: Pg/Gcs/Gke/Redis adapters + operator-admin + cloud `main.ts`. It moved to its private home, which vendors this repo at a pinned SHA; a seam change lands here first (ports in `packages/host`), then the closed side bumps its pin. The open side of the seam (open code never imports a cloud adapter or `@houston/host-cloud`) is documented in `BOUNDARY.md` and enforced by `scripts/check-boundaries.mjs` (`pnpm check:boundaries`, wired into the PR CI gate `.github/workflows/ci.yml`).
