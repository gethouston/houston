...
- `glow-{blue,indigo,orange,amber}` + `glow-blue-wash`/`glow-blue-shadow`: the running comet (card glow, avatar ring, progress line), theme-invariant, consumed only by the `.card-running-glow` / `.avatar-running-ring` / `.running-glow-line` recipes in `ui/core/src/globals.css`; never as decorative colour elsewhere.
