...
- **Agent linkage**: each agent's `.houston/activity/activity.json` activity carries the `claude_session_id` it chatted under. So: for each agent → each activity with a `claude_session_id` → pull its `chat_feed` rows. *(Assumption to verify when building: the Rust activity records do carry `claude_session_id`. If some chats have no activity, they're orphans — log + skip, no silent drop.)*
