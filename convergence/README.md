...
Remaining in P3: **3.4c** — the run-completion hook (transition runs running→surfaced/silent by observing turn completion + the `ROUTINE_OK`/`suppress_when_silent` classification). Today a fired run is recorded "running" by the scheduler and flips to "error" only if firing is rejected. Then: workspaces family, portable agents, migration-chain port.
