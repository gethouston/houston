...
Remaining in P3: **3.4b** — wire the Scheduler into `main` with a REAL firer (needs a new `RuntimeChannel.fireTurn(ctx, conversationId, text)` method on both channels — Proxy posts to the runtime, Turn extracts the `startTurn` core — plus the relay turn-completion hook that transitions runs running→surfaced/silent). Then: workspaces family, portable agents, migration-chain port. The Scheduler class is built + tested against a fake firer but NOT yet constructed in `main` (the live-turn-path integration is isolated to 3.4b on purpose).
