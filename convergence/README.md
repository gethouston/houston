...
- **P4 — Local profile.** 🟡 in progress. Done: `ProcessLauncher` (subprocess `RuntimeLauncher` — lazy spawn one pi-runtime per agent, warm reuse, SIGTERM on sleep, zombie-reap on never-healthy; `BunRuntimeSpawner` is the production path) — combined with the EXISTING `ProxyChannel`, local dispatch is the same channel code as cloud, different launcher. `FsWatcher` + pure `classifyChange` (path → HoustonEvent, matches `houston-file-watcher`, routine_runs-vs-routines prefix handled) feeds the same `EventHub`. Remaining: local host entry point (wire ProcessLauncher + SingleUserVerifier + FsVfs + FsWatcher + in-process bus + local connect-once credentials), `packages/control-plane` → `packages/host` dir rename (with cloud deploy/k8s refs), host-as-sidecar packaging.
