 ...
 *
 * The reveal posture of `FormDialog`: the key is already minted, so there is
 * nothing left to cancel and the acknowledging primary is the only way out.
 */
export function WebhookKeyDialog({ revealed, onClose }: Props) {
  ...
  return (
    <FormDialog
      ...
      title={t("webhook.dialog.title")}
      description={t("webhook.dialog.subtitle")}
      primary={{ label: t("webhook.dialog.done") }}
      secondary={null}
      labels={{ close: t("webhook.dialog.close") }}
    >
      {revealed && (
        <>
          ...
          <p className="flex items-start gap-2 text-danger text-xs">
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2 rounded-lg border border-line bg-input px-3 py-2">
        <code className="min-w-0 flex-1 break-all font-mono text-ink text-xs">
          {value}
        </code>
