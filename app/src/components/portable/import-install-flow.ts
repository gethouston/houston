 ...
 * it: refuse an unusable name, run the round-trip, reveal the agent, and only
 * then let it start its own setup. `use-import-install-action.ts` supplies the
 * effects and wraps this in the same-tick latch.
 *
...
export interface ImportInstallRun {
  ...
  /** The agent's own setup, which runs with the sheet already gone. */
  startSetup: (
...
export async function runImportInstall(run: ImportInstallRun): Promise<void> {
  ...
  try {
    ...
    run.startSetup(installed, kickoffPin);
  } catch (err) {
