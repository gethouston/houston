 ...
 * it: the role-aware blocked-state CTAs in the agent workspace (a locked/
 * forbidden app a manager CAN enable) send the user to the Agents tab, or
 * straight into that agent's per-agent card. Rather than lift that state into
 * the shared UI store, this tiny colocated store carries the intent: the caller
 * sets the request then switches `viewMode` to the Permissions view;
 * `PermissionsView` consumes it (initial mount AND while already open) and clears
 * it so a later plain nav lands on the default tab.
 */
interface PermissionsNavState {
  ...
  /** Ask the view to open `tab` (consumed + cleared by the view). */
  requestTab: (tab: PermissionsTab) => void;
