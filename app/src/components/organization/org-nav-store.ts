 ...
 *
 * The dashboard owns its own tab state, but deep links arrive from OUTSIDE it:
 * the C8 team-status banner/trial pill (in the shell) sends the user to the
 * Billing tab. Rather than lift tab state into the shared UI store (and couple
 * every consumer to it), this tiny colocated store carries the intent: the
 * caller sets `requestedTab` then switches `viewMode` to the org view;
