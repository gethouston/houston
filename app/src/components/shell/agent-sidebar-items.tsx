 ...
 * edge and nothing after it: copying, publishing and deleting an AI Employee
 * live in its Settings, so the row stays one quiet target. The unread dot
 * stays absent.
 *
...
export function buildAgentSidebarItems({
  ...
  return agents.map((agent) => {
    ...
    return {
      id: agent.id,
      name: agent.name,
      // The role the host names on the listing: no job description is read
      // here, so opening the app never reaches (or wakes) an employee's pod.
      subtitle: agent.role,
      icon: (
        <AgentSidebarIcon
          color={agent.color}
