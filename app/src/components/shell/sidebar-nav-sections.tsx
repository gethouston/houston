 ...
 *
 * The Assistant, AI Models and Integrations are the three things a user
 ...
 * assistant exists. "Your teams" is the rail's only labelled band, drawn
 * below by the teams model.
 *
 ...
 * preference rather than a place work happens. The shared Skills library is
 * the Skills TAB of the Integrations row below: what every agent in the space
 * can do belongs beside the apps they can reach, not in a row of its own.
 * About me is a Settings section as well: what the agents know
 * about the person is kept with their name and their language. "Guide me" is
 * one of two items behind the help control in the rail's footer
 * (`sidebar-help-menu.tsx`), because it points at no screen. The Academy and
 * Settings are the rail's FOOTER cluster (`sidebar-footer.tsx`): learning to
 * fly and the person's own chrome sit under the space's contents rather than
 * above them.
 */
export function buildSidebarNavItems(args: {
  ...
  const { t, showAiModels, showAssistant, setViewMode } = args;
  const { assistant, aiModels } = gatedNavRows({ t, setViewMode });
  return [
    {
      id: "primary",
      items: [
