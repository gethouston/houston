...
/**
 * Toggle ONE connected account in ONE agent's grant set, for the global page's
 * detail sheet where the agent is chosen at click time (so the per-agent
...
export function useAgentGrantToggle() {
  ...
  return useMutation({
    mutationFn: ({ agentId, connectionId, active }: ToggleVars) => {
    ...
    onMutate: async ({ agentId, connectionId, active }) => {
      const key = queryKeys.agentGrants(agentId);
    ...
    onError: (_err, { agentId, connectionId, active }) => {
      const key = queryKeys.agentGrants(agentId);
      const change: GrantChange = {
        connectionId,
        op: active ? "add" : "remove",
