...
When you have everything, summarize the skill in a few plain lines (what it does, and when you'll use it) and ask for approval with ask_user (Yes / No). Only create it after a Yes. Create it following your Skills guidance — propose a short plain name yourself — and include a frontmatter field "setup_activity_id" set to exactly "${activityId}"; that keeps this chat attached to the skill. Also keep the Houston-created metadata and the versioned Workflow marker from the Skills guidance, because Houston uses that Workflow section to show the user clear steps. Every step that uses one of their connected apps carries that app's tag, exactly as your Skills guidance describes, so running the skill later goes straight to the right app action. Never mention these fields or any other technical detail to the user. Then confirm it is ready, mention they can run it any time just by asking, and remind them they can change it right here, in this same chat, any time.`;
}
...
- The "x_houston" metadata block and the versioned Workflow marker identify Houston-created skills. Keep them unchanged if present. The Workflow section is what Houston shows as clear steps, so update it whenever the user's change alters the action flow. A step that uses a connected app carries that app's tag from your Skills guidance: when a change moves a step to a different app or a different action, update that step's tag too, and keep the frontmatter "integrations" list in step with it.
