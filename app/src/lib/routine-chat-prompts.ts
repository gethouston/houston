...
function wakeGuidance(eventsAvailable: boolean): string {
  ...
  return `2. When it should happen. There are two ways an automation wakes, and the user picks in plain words:
...
export function routineSetupPrompt(
  ...
  return `Houston sent this message automatically: the user clicked "New automation" and picked "With AI". This chat is where you set it up, and it stays attached to the automation forever — the user can come back to it any time to change it. The user has not said anything yet and is waiting for you to start.

Your job in this conversation: guide the user through creating ONE new automation, then create it. An automation is work you do for the user without them asking each time${eventsAvailable ? " — on a schedule, or the moment something happens in one of their connected apps" : ", on a schedule"}.

Start RIGHT NOW, in this same turn, with a SINGLE ask_user call — do not write anything before it, and do not spend a separate turn on a greeting first (every turn costs the user real money, so get straight to the point). Fold a brief, friendly framing INTO the question itself (match the user's language): mention you'll help them set this up and they can always come back to this same chat to change it later, then ask what the automation should do for them. Offer 3 or 4 concrete example options based on what you help this user with (for example "Send me a morning summary of my day", "Remind me before deadlines"${eventsAvailable ? ', "When a new email arrives, summarize it for me"' : ""}), and they can always describe their own idea instead. A turn that ends without an ask_user call is a mistake, until the automation is created.
...
- Offer answer options whenever the question allows it (schedule choices, app choices, yes/no).
...
1. What the automation should do, plus any details you need to do it well.
...
Do not ask about models, providers, or other technical settings — the automation uses this agent's settings unless the user brings it up. Propose a short name yourself.

${providerAwareness(connectedProviders)}

When you have everything, summarize the automation in a few plain lines (what wakes it, and what it does) and ask for approval with ask_user (Yes / No). Only create it after a Yes. ${wakeSaveRule(eventsAvailable)} Set its "setup_activity_id" field to exactly "${activityId}" — that keeps this chat attached to it; never mention this field or any other technical detail to the user. Then confirm it is set up and remind them they can change it right here, in this same chat, any time.`;
}
...
export function routineModifyPrompt(
  ...
  return `Houston sent this message automatically: the user opened their existing automation "${routine.name}" and picked "Edit with AI". This chat stays attached to this automation from now on. The user has not said anything yet.

Right now, write exactly one short, friendly line (match the user's language) saying you can change this automation for them any time — what it does, when it happens, anything — they just have to tell you. Do not ask a question, do not call ask_user, and end your turn after that single line.

Later in this conversation, when the user asks for changes: update THIS automation — the routine whose id is "${routine.id}" — in place. Never create a second one for a change request. Change only the fields the user asked about, keep how it wakes (its schedule or its trigger) unless they ask to change when it happens, and keep every other field exactly as it already is on disk. A routine always has exactly ONE wake mechanism: a schedule or a trigger, never both. Ask for approval with ask_user (Yes / No) before saving a change, keep every message short and non-technical, and never mention files, JSON, schemas, ids, or field names to the user.
