...
use crate::provider::CodexBackend;
use crate::Provider;
...
/// name via `-p`.
///
/// `provider` lets a [`CodexBackend`] provider (OpenRouter, …) ride this
/// same builder: when `provider.codex_backend()` is `Some`, the
/// model-provider `-c` overrides are emitted and its `default_model` fills in
/// when the agent hasn't picked one. Native codex (OpenAI) returns `None`, so
/// not a single argv byte changes for it.
pub(crate) fn build_args(
    ...
    // Custom OpenAI-compatible backend (OpenRouter, …): point codex at the
    // provider's endpoint via `-c` overrides (highest precedence). Empty for
    // native codex.
    let backend = provider.codex_backend();
    if let Some(ref b) = backend {
        args.extend(codex_backend_overrides(b));
    }

    // The agent's pick wins; otherwise fall back to the backend's default so
    // a freshly connected OpenRouter agent can chat without choosing a model.
    let model = model.or(backend.and_then(|b| b.default_model));
    if let Some(m) = model {
...
/// `-c model_provider` / `model_providers.<slug>.*` overrides describing a
/// custom OpenAI-compatible endpoint. The API key is NOT here — it rides as
/// the `env_key` environment variable, injected by the runner via
/// [`crate::provider::codex_backend_env`].
fn codex_backend_overrides(b: &CodexBackend) -> Vec<OsString> {
    b.config_overrides()
        .into_iter()
...
mod tests {
    ...
    #[test]
    fn native_codex_emits_no_model_provider_override() {
        let args = strings(build_args(openai(), None, None, Some("gpt-5.5"), None, None));
        assert!(
            !args.iter().any(|a| a.starts_with("model_provider")),
            "native codex must not carry a model_provider override: {args:?}"
        );
    }

    #[test]
    fn openrouter_emits_process_local_provider_overrides() {
        let args = strings(build_args(openrouter(), None, None, Some("x-ai/grok-2"), None, None));
        for expected in [
            ...
            assert!(
                ...
                "missing override {expected:?} in {args:?}"
            );
        }
    }

    #[test]
    fn openrouter_without_model_uses_backend_default() {
        let args = strings(build_args(openrouter(), None, None, None, None, None));
        let pos = args
            .iter()
            .position(|arg| arg == "--model")
            .expect("--model should be present");
        assert_eq!(args[pos + 1], "deepseek/deepseek-chat");
    }
