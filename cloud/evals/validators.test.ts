...
test("validatePptx fails on too few slides and on non-zip bytes", async () => {
  ...
  const garbage = await validatePptx(new TextEncoder().encode("not a zip"), { minSlides: 4 });
  expect(garbage.find((c) => c.name === "valid zip container")!.pass).toBe(false);
});
...
test("validateXlsx checks workbook skeleton and data rows", async () => {
  const good = await zipOf({
    ...
    "xl/worksheets/sheet1.xml": "<worksheet><row r=\"1\"/><row r=\"2\"/><row r=\"3\"/></worksheet>",
  });
  ...
  expect(checks.find((c) => c.name === "worksheet has data rows")!.pass).toBe(false);
});

test("validatePng checks magic header and minimum size", async () => {
  const png = new Uint8Array(6000);
  png.set([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a], 0);
  expect((await validatePng(png, { minBytes: 5000 })).every((c) => c.pass)).toBe(true);

  const tiny = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 1, 2]);
  ...
  expect((await validatePng(notPng, { minBytes: 1 })).find((c) => c.name === "PNG magic header")!.pass).toBe(
    false,
  );
});
