...
| POST | `/v1/providers/openrouter/credentials` | Write `OPENROUTER_API_KEY` to `<houston-home>/providers/openrouter/.env` (atomic, mode 0600). Body: `{ apiKey }`. The codex runner injects it into the subprocess for OpenRouter's `model_providers.openrouter` config. TS client: generic `setProviderApiKey(id, key)` → `POST /providers/<id>/credentials`. |
