...
| Store frontend | `agentstore/` | Next.js 15 App Router SSR catalog. **No database, no service credentials**; every read/write goes to the gateway `/v1/agentstore/*`. Sign-in is GCIP (Firebase Auth). Ships as a standalone container to GKE. **Space theme only**: `data-theme` is pinned dark (no toggle), body is transparent over the fixed Milky Way layers (`src/app/space.css` + `components/space-background.tsx`, static — no parallax, see the perf note there). Typography matches gethouston.ai (General Sans display via fontshare, system-stack body — no body webfont). Navigation is the SHARED `StoreNav` from `@houston-ai/store` (fixed content: brand · theme · account) with web behavior injected; the account control is the site's `UserMenu` avatar dropdown (Sign in / Your agents / Sign out) injected via `accountSlot` — the nav is the ONE identity control, pages never render a second avatar. The older `SiteHeader` survives only on the `admin` and `claim` pages. Share cards: `app/opengraph-image.tsx` (default) + `app/a/[slug]/opengraph-image.tsx` (per-agent, via `lib/og-card.tsx`) render space-styled OG images with next/og. The website's top menus link here (`https://agents.gethouston.ai`). |
