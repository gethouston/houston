...
| Path | Source of truth | Worker reads from | Worker writes to | Projected doc | Served asleep from | Claim | Flags |
|---|---|---|---|---|---|---|---|
| `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `WORKSPACE.md`, `USER.md` | store | T, O(route): hydrate. The runtime loads exactly one of `AGENTS.md` / `CLAUDE.md` (AGENTS first) via `agentsFilesOverride`; `WORKSPACE.md` and `USER.md` via `workspace-context.ts`, replaced by request fields on cloud turns; `GEMINI.md` is never read. Symlinked mirrors are not in the store | T: never, even when the model edits it. O(route): store via `PUT agentfile/<path>` | none (`ContextChanged` has no family) | `GET agentfile/<rel>`: O(route) read; `PUT`: O(route) | agent-ops | D1 |
| `GROUP.md` | cloud: the team's shared context row in Postgres; the file is a mirror the gateway writes into awake pods via `PUT agentfile/GROUP.md`. Desktop and self-host: derived from the `sidebar_layout` preference by `group-context-sync.ts` | T, O: hydrate | T: never. O: not written by any op; sleeping pods are skipped | none | re-driven on the next wake, at provisioning, and on a cross-org move | none | D14 |
...
| user files (everything else under the agent root) | store | T, O(route): hydrate. O(settings/credential): `files/` and `uploads/` excluded | T: never. Every file the model writes in a pool turn is dropped at sync-back. O(route): store (`files/*` import, move, rename, folder, delete) | none | `files/*` GET, POST, DELETE incl. listing and download: O(route) on a worker; binary rides base64, 100 MiB cap on the declared content length. `files/*` PUT or PATCH and `skills/*` GET are unclassified and wake | agent-ops | D1 D13 D17 |
...
| `.houston/agent.json`, `.houston/sessions/**`, `.houston/integrations.json`, `.houston/memory/`, `.houston/prompts/`, flat `.houston/<f>.json` | legacy (Rust era, pre-v0.4). The boot layout migration copies flat files and `memory/learnings.md` forward and leaves the originals; only `prompts/*.md` is deleted | hydrate | O(route) scope admits them but nothing writes | none | n/a | - | W1b |
