...
| D2 | `settings.json` / `custom-endpoint.json` vs the `providers` view doc | A settings op lands the file but returns `events: []`, so `republish()` captures nothing. The gateway overlay recomputes `isActive` from the current `settings.json` and credential store (first connected provider when no pool dispatcher exists), but `activeModel` and the `openai-compatible` row (no credential-store row) stay as captured | Next pod wake (`warmViewDocs`) or a live client `GET providers` |
...
| D8 | transcript rows vs conversation file | The doc shadow seeds at boot; the transcript shadow does not. In-process repairs happen only on send failure or queue overflow. A pod that sleeps after a failed send leaves Postgres stale until the next mutation of that conversation | No pod-boot seed. The control-plane transcript backfill sweep (every 60 s, leader-elected) re-derives rows from the conversation object and repairs absent or mismatched rows when armed; unarmed it only reports |
| D9 | chat-history boot migration | `migrateChatHistory` writes conversation files at every boot with no transcript enqueue; under `database` authority the gateway list will not show them until mutated | The armed backfill sweep (D8) |
| D10 | adopted claim | An adopted re-run appends a second user/assistant pair behind the dead worker's; `HttpTurnTranscript` picks the last by `turnId`. The file keeps both, Postgres keeps the last | The armed backfill sweep (D8), which makes the file win: both pairs land in Postgres |
...
| D17 | `runtime.log` treated as state | Inside the data dir, excluded by nothing: uploaded whenever it changed (most ticks on a busy pod), downloaded on every turn and non-route op hydrate, counted against the 2 GiB cap | None |
...
| D19 | 413 over-cap files (store per-object cap 256 MiB) | Recorded in `skipped[]` with their hash so they never retry until they change; the file exists on the pod and nowhere else. A worker never sees it | None |
| D20 | partly durable op | Four outcomes in `execute-op.ts`: nothing landed answers `decline` (the gateway proxies and wakes); something skipped as over-size answers 413; something landed while other objects conflicted or were out of scope answers `ok: true, ambiguous: true` (a re-run would duplicate the landed half); otherwise ok | None by design; the client refreshes and decides |
...
| D23 | any pool claim (agent-ops or conversation id) vs the agent write lease | `fenced` short-circuits to the claim path when claim headers are present, so a worker's write bypasses the lease the awake pod holds and is bounded only by its own claim scope | Ordering from D15; generation preconditions detect but do not protect |
...
| D25 | per-row `conversations.authority` vs per-agent phase | Promotion updates both in one transaction, but `GetAgentAuthority` aggregates rows: all `database` reads `database`; any `shadow` row demotes the agent to `shadow`; a `file` row with no shadow row (or zero rows) reads `file`. One divergent row silently demotes the whole agent | Inheritance at insert; fails safe but invisible |
...
- **Lazy hot-set hydrate.** The per-path hydrate columns above are the hot
  ...
  files the model opens. Pool turns also run with `integrations`,
  `saveRoutine`, `saveLearning` and `skillDirectory` off. Everything else
  hydrated today is either never read
...
- **SSE fan-out.** Op-made changes that already emit a host event on the
  ...
  `ContextChanged`, `ConversationsChanged`) leave the worker as event types
  in the op reply (`execute-op.ts`); the gateway's reply struct has no
  `events` field, so they are discarded on decode. Writes that emit nothing anywhere, so fan-out alone will
