...
- **Release.yml uploads:** After Tauri build (which has already injected the bundle), the macOS job runs `sentry-cli releases new + set-commits + sourcemaps upload + debug-files upload + finalize` against `target/{aarch64,x86_64}-apple-darwin/release/houston-engine`. Each Windows matrix arch uploads its own `app/dist` maps (Vite content-hashes differ per arch) + `houston-engine.exe` + `houston_engine.pdb` (PDB filename has underscore — Rust convention). The CI steps **upload only** (no `inject` — that now happens at build time). Skipped silently when `SENTRY_AUTH_TOKEN` is unset (forks, personal builds).
