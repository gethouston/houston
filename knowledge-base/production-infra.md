...
- **Breadcrumbs:** `sentry-tracing` layer wired into BOTH `app/src-tauri/src/logging.rs` (app process) and `engine/houston-engine-server/src/main.rs::init_tracing` (engine process). Every `tracing::info!`/`warn!`/`error!` becomes a breadcrumb on that process's subsequent Sentry events (ERROR also fires a standalone event). Last ~100 log records auto-ride with each crash. **Privacy posture (deliberate, beta):** breadcrumbs AND event messages are intentionally NOT scrubbed — they can leak binary paths and agent names. We accept this tradeoff for crash-debug value during beta (the visible Session Replay IS masked; this is about the crash payload, not the recording). Revisit by adding a `sentry_tracing::layer().event_mapper(...)` + a `before_send` scrubber on both the JS and Rust clients together — do NOT ship a partial scrubber that cleans the title but leaves breadcrumbs. **Volume note:** every `tracing::error!` becomes a standalone Sentry event. Most engine error sites are one-shot, but a persistently-failing scheduled routine (`engine/houston-engine-core/src/routines/scheduler.rs`) can emit one event per cron fire. These now collapse: both clients stamp a normalized `fingerprint` in `before_send` (see **Issue grouping / dedup** below), so a noisy site lands as ONE issue regardless of the volatile data in its message. A site that should be silent entirely still wants a `sentry::with_scope`/`event_filter` downgrade, but you no longer need per-site fingerprinting just to stop duplicate-issue sprawl.
- **Issue grouping / dedup (HOU-449):** Both clients stamp a deterministic `fingerprint` in their `before_send` (Rust) / `beforeSend` (JS) hook so a family of near-identical messages collapses into ONE issue instead of one-per-variant. The normalizer — `app/src/lib/sentry-fingerprint.ts` for the renderer, `engine/houston-engine-server/src/sentry_grouping.rs` for the engine, kept in sync — takes the FIRST line of the event message and masks volatile tokens (IPv4`:port`, UUIDs, `0x` hex, bare digit runs → `{addr}`/`{uuid}`/`{hex}`/`{n}`), then uses that string as the single fingerprint element. It only overrides Sentry's DEFAULT fingerprint, so a deliberate `scope.set_fingerprint` (the supervisor's `engine-subprocess-exit`) still wins, and it no-ops on an empty message (keeps default grouping). This killed the two dominant duplicate families: the renderer's `<command>: Failed to fetch (127.0.0.1:<random port>)` transport drops (the sidecar port changes every restart, so ~40 issues were really a couple) and the engine's `Failed to parse Codex event: … at line 1 column <N>` parser errors (the serde column moves every malformed frame AND the raw frame is appended on a second line, so ~50 issues were one bug). The app *process* (Rust, `lib.rs`) is intentionally NOT fingerprinted: its error messages (updater URL, WebView2 HRESULT, panics) are constant and already land as one issue each — add the same `before_send` there if that ever changes. **Existing backlog:** the code fix only stops NEW dupes; merge the historical duplicates by hand in the Sentry UI (Issues → multi-select → Merge — irreversible, so it's deliberately not scripted).
- **Resolved-issue recurrence / regressions (HOU-449 investigation):** Sentry keys resolve→reopen entirely off the FINGERPRINT. When an event whose fingerprint matches a *resolved* issue arrives, Sentry flips it back to Unresolved and records a **regression** (Activity tab, plus a regression alert if one is configured). Plain "Resolve" reopens on ANY later matching event; "Resolve in next release" stays resolved until a matching event lands in a release semver-greater than the one current at resolve time (older / same-release events do NOT reopen it); "Archive/Ignore" only mutes notifications and auto-unarchives on escalation, not as a regression. The catch — and why this shared a ticket with dedup — is that an UNSTABLE fingerprint breaks all of it: a recurrence that arrives with a different port/column gets a brand-new issue instead of reopening the resolved one, so you can never tell the bug came back. The deterministic fingerprints above are the prerequisite that makes resolve→regression a trustworthy signal that a fix held.
- **Auto-report flow:** `app/src/lib/error-toast.ts` shows a red "Houston, we have a problem" toast immediately, captures the real `Error` (the original stack — `tauri.ts::surfaceError` forwards it so the event shows the real engine stack, not the `showErrorToast` frame; grouping itself is governed by the normalized `fingerprint`, see **Issue grouping / dedup**), waits for delivery confirmation, then shows a green "Houston, report sent" toast with the event ID prefix + a "Copy code" action that copies the FULL 32-char id (so it can be quoted to support / looked up in Sentry). The id is surfaced ONLY on a confirmed 2xx from Sentry over the direct fetch transport (the wrapper in `sentry.ts` records per-event HTTP status; `captureException` gates on flush AND accept) — so the toast cannot show an id Sentry didn't accept. (Before the direct-HTTP switch the IPC transport returned 200 unconditionally, which is why the toast could lie — see the Frontend bullet.) Capture is decoupled from the toast: `{ toast: false }` engine calls still report to Sentry unless they also pass `{ capture: false }` (fire-and-forget / self-reporting paths); `AbortError`s are filtered (cancelled requests aren't failures). User never has to click "Report bug" when Sentry is reachable.
