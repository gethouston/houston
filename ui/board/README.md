...
Cards matching `runningStatuses` get the `card-running-glow` CSS class: a `conic-gradient` border animated with `@property --glow-angle`, spinning at 2.5s. Blue, indigo, orange, gold, from the `--ht-glow-*` tokens. Pure CSS, no JS animation loop. The recipe lives in `@houston-ai/core`'s `globals.css`, which every consumer already imports.
