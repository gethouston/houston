...
| `conversations.delete` | DELETE | unconfirmed: withheld from dispatch | it acts on the chat the person has open, and nothing lists an agent's chats, so a dispatched call has no id it could name. | agentId: resolved:agents; id: open: A chat is the one the person is looking at, and nothing lists an agent's chats. |
| `conversations.rename` | PATCH | unconfirmed: withheld from dispatch | it acts on the chat the person has open, and nothing lists an agent's chats, so a dispatched call has no id it could name. | agentId: resolved:agents; id: open: A chat is the one the person is looking at, and nothing lists an agent's chats.; title: free text |
| `turns.cancel` | unroutable | unconfirmed: Stops work already under way; nothing already said or written is undone. | visible | conversationId: open: A chat is the one the person is looking at, and nothing lists an agent's chats.; agentId: resolved:agents |
...
| `integrations.connect` | unroutable | unconfirmed: withheld from dispatch | starts a browser sign-in only the user can finish; the assistant queues the connection card with request_connection instead. | toolkit: open: Toolkits belong to the integration provider, so read the slug from integrationToolkits. |
